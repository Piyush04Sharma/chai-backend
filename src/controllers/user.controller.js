...
const registerUser = asyncHandler(async (req, res) => {
    ...
    if(!avatar){
        throw new ApiError(500, "Avatar upload went wrong");
    }
1
    ...
    if(!createdUser){
        throw new ApiError(500, "User creation went wrong");
    }
...
const loginUser = asyncHandler(async (req, res) => {
    ...
    if(!username && !email){
        throw new ApiError(400, "One is required");
    }
...
const getUserChannelProfile = asyncHandler(async (req, res) => {
    ...
    if(!channel?.length){      //if(channel.length === 0){ 
